...
[GordianNode-macOS.app](https://github.com/BlockchainCommons/GordianNode-macOS) is a personal one-click installer for Bitcoin Core and Tor that will present a QuickConnect QR code that can be used to pair mobile wallets for remote use over Tor V3.
...
[Linux Bitcoin-StandUp-Scripts](https://github.com/BlockchainCommons/Bitcoin-StandUp-Scripts) achieve the same thing as GordianNode-macOS, but come in the form of Linux scripts.
...
[Quick Connect API](Docs/Quick-Connect-API.md) defines the spec for a deep link URI and a scannable QR Code. It is used by *GordianNode-macOS* as well as by several server-side node manufacturers.
...
3. [Security for GordianNode-macOS](Docs/Security.md). Notes on ensuring the security of your *GordianNode-macOS* node.

### Related Projects

The full node created by *GordianNode-macOS* can also be interlinked with other projects from Blockchain Commons.

1. [GordianWallet-iOS](https://github.com/BlockchainCommons/GordianWallet-iOS) is a state-of-the-art mobile wallet that was made to work with a *GordianNode-macOS* full node.  You can scan the QuickConnect QR code from *GordianNode-macOS* and easily create mutli-sig wallets where one key gets stored on your device, one on your node, and one in offline backup.
2. [Learning Bitcoin from the Command Line](https://github.com/ChristopherA/Learning-Bitcoin-from-the-Command-Line) is a tutorial for programming the `bitcoin-cli`, which can be run using a full node such as the one created by *GordianNode-macOS*.
